...
rules.py, items.py, world.py) never needs to know or care about "levels" or
"hubs" as concepts.
...
REGIONS: list[str] = ["menu"] + [f"level{i + 1}" for i in range(5)]
...
CONNECTIONS: list[Connection] = [
  {
    "from_region": "menu",
    "to_region": f"level{i + 1}",
    "name": f"menu to level{i + 1}",
    "requires": [[f"level:level{i + 1}"]],
  }
  for i in range(5)
]
...
EVENTS: list[EventDef] = [
  # {
  #   "room": f"level{level_index}",
  #   "location_name": f"level{level_index} - star {star_index} can be got",
  #   "item_name": "flag:starCanBeGot",
  #   "requires": None,
  # }
  # for level_index, star_count in enumerate([1, 3, 2, 2, 1, 2, 3, 1, 2, 3, 6])
...
NON_POOL_PREFIXES: tuple[str, ...] = ("flag:",)
