...
#
# Run this from the Archipelago repo root's worlds/mathbreakers directory, or adjust
# WORLD_DIR below.
set -euo pipefail
...
# Candidates to test as the forced item for menu - level:level1
STAGE0_CANDIDATES=(
  "level:level1"
  "level:level2"
  "level:level3"
...
# level0_BEST_ITEM you lock in first -- see main())
...
#   set_forced_items "menu" "level:level1" "move:walljump" "menu" "achievement:30:MICROWAVE" "move:bounce"
set_forced_items() {
...
run_fuzz() {
  # Runs from the Archipelago repo root (two levels up from worlds/mathbreakers).
  (python3.13 ../../fuzz.py -r "$RUNS" -j "$JOBS" -g mathbreakers -n 1) 2>&1 || :
}
...
main() {
  ...
  sweep_slot "menu" "level:level1" "${STAGE0_CANDIDATES[@]}"
  print_ranking "level:level1" "${STAGE0_CANDIDATES[@]}"
  compute_weights "level:level1" "${STAGE0_CANDIDATES[@]}"
